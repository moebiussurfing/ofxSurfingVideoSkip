...
void ofxSurfingVideoSkip::setup()
{
	...
	path_fileName_AppSettings = "ofxSurfingVideoSkip_App_Settings.xml";

#ifndef USE_ofxPresetsManager_Hap
	path_fileName_Engine = "ofxSurfingVideoSkip_Engine_Settings.xml";
...
#define SPEED_MAX 50.0f
	...
	TRIG_Reset_Mods.set("RESET ENGINE", false);
	...
	TRIG_reverse_Skiper.set("TRIG REV", false);
	...
	TRIG_Reset_Mods.setSerializable(false);
	...
	SHOW_Skipper.set("SHOW HAP-SKIPPER", true);
	...
	_param_SkipEngine.add(TRIG_reverse_Skiper);

	_param_SkipEngine.add(TRIG_Reset_Mods);
	...
	params_Engine_Store.setName("ofxSurfingVideoSkip");
	params_Engine_Store.add(POSITION_Start);
	params_Engine_Store.add(POSITION_Finish);

	params_Engine_Store.add(speed_Reset);
	params_Engine_Store.add(speedNorm);
	params_Engine_Store.add(speed);

	params_Engine_Store.add(loopedBack);
	params_Engine_Store.add(reverseSpeed);
	params_Engine_Store.add(ENABLE_TimersSkipRev);

#ifndef USE_BPM_TIMER_MODE
	params_Engine_Store.add(timePeriod_skipper);
	params_Engine_Store.add(timePeriod_reverser);

	params_Engine_Store.add(MODE_SkipTime);
	params_Engine_Store.add(tBeatSkipper);
	params_Engine_Store.add(MODE_SkipReverse);
	params_Engine_Store.add(tBeatReverse);
#else	
	params_Engine_Store.add(bpmTimer);
	params_Engine_Store.add(bpmDivider);
	...
	params_Control.add(SHOW_Skipper);
	...
	moodMachine.setGui_AdvancedVertical_MODE(true);

	//listeners for inside class moodMachine
	moodMachine.RANGE_Selected.addListener(this, &ofxSurfingVideoSkip::Changed_Mood_RANGE);
	moodMachine.TARGET_Selected.addListener(this, &ofxSurfingVideoSkip::Changed_Mood_TARGET);
	moodMachine.PRESET_A_Selected.addListener(this, &ofxSurfingVideoSkip::Changed_Mood_PRESET_A);
	moodMachine.PRESET_B_Selected.addListener(this, &ofxSurfingVideoSkip::Changed_Mood_PRESET_B);
	moodMachine.PRESET_C_Selected.addListener(this, &ofxSurfingVideoSkip::Changed_Mood_PRESET_C);
#endif
...
void ofxSurfingVideoSkip::startup()
{
	...
	{
#ifndef USE_ofxPresetsManager_Hap
		loadGroup(params_Engine_Store, path_GLOBAL_Folder + "/" + path_fileName_Engine);

		//#ifdef USE_ofxChannelFx
		//		loadGroup(params_Engine_Store, path_GLOBAL_Folder + "/" + path_fileName_ChannelFX);
		//#endif
#endif

		loadGroup(params_App, path_GLOBAL_Folder + "/" + path_fileName_AppSettings);
...
void ofxSurfingVideoSkip::setGuiVisible(bool b)
{
	...
	//ofxSurfingMoods
#ifdef USE_ofxSurfingMoods
	moodMachine.setGui_Visible(b && SHOW_MoodMachine);
#endif
}
...
//	moodMachine.setPosition(gPos.x + gWidth + xPad, gPos.y);//right to the user panel
//
//	//float pw = 400;
//	//float px = window_W * 0.5f - pw * 0.5f;//center on screen
//	//moodMachine.setPreviewPosition(px, 20, pw, 40);//custom preview position
//#endif

	//-

//#ifdef USE_ofxSurfingMoods
//	auto gGenWidth = moodMachine.getGuiUserWidth();//right to the moodMachine panel
...
void ofxSurfingVideoSkip::setup_PresetsManager()
{
	...
	presetsManager.add(params_Engine_Store, { '0', '1', '2', '3', '4', '5', '6', '7', '8' });
...
void ofxSurfingVideoSkip::Changed_params(ofAbstractParameter &e) //patch change
{
	if (!DISABLE_CALLBACKS)
	{
		...
		else if (name == TRIG_Reset_Mods.getName() && TRIG_Reset_Mods)
		{
			TRIG_Reset_Mods = false;
...
{
	ofLogNotice(__FUNCTION__) << ofToString(SHOW_gui.get());
...
//	//
//	//#ifdef USE_ofxSurfingMoods
//	//				//moodMachine.setGui_visible(ENABLE_drawVideoControlBar&&SHOW_MoodMachine);
//	//#endif
//	//			}
//	//			else if (MODE_App == 1)
//	//			{
...
//moodMachine
//listeners for inside moodMachine
...
void ofxSurfingVideoSkip::gui_CustomizeApply()
{
	...
	{
		//panels control
		(panel_Control->getToggle(SHOW_Skipper.getName()))->setConfig(jConf_BigBut2);
#ifdef USE_ofxChannelFx
...
void ofxSurfingVideoSkip::exit()
{
	...
	//save app settings
	saveGroup(params_App, path_GLOBAL_Folder + "/" + path_fileName_AppSettings);

#ifndef USE_ofxPresetsManager_Hap
	saveGroup(params_Engine_Store, path_GLOBAL_Folder + "/" + path_fileName_Engine);
#endif
...
//#ifndef USE_ofxPresetsManager_Hap
//	saveGroup(params_Engine_Store, path_GLOBAL_Folder + "/" + path_fileName_ChannelFX);
//#endif
...
#ifdef USE_ofxSurfingMoods
	//listener for inside class moodMachine
	moodMachine.RANGE_Selected.removeListener(this, &ofxSurfingVideoSkip::Changed_Mood_RANGE);
	moodMachine.TARGET_Selected.removeListener(this, &ofxSurfingVideoSkip::Changed_Mood_TARGET);
	moodMachine.PRESET_A_Selected.removeListener(this, &ofxSurfingVideoSkip::Changed_Mood_PRESET_A);
	moodMachine.PRESET_B_Selected.removeListener(this, &ofxSurfingVideoSkip::Changed_Mood_PRESET_B);
	moodMachine.PRESET_C_Selected.removeListener(this, &ofxSurfingVideoSkip::Changed_Mood_PRESET_C);
#endif
}
	...
	guiManager.begin();
	{
		...
		// windows
		{
			...
			guiManager.beginWindow(n.c_str(), &bOpen0, flagsw);
			{
				...
				widgetsManager.Add(MODE_EDIT, SurfingWidgetTypes::IM_TOGGLE_BIG, 0, 0, 5);
