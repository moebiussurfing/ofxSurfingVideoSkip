...
private:
	void Changed_Params(ofAbstractParameter & e) // patch change
	{
		...
		else if (name == volumeAudio.getName()) {
		...
		else if (name == positionAudio.getName()) {
			if (!bLinkAudio)
		...
		else if (name == bLinkAudio.getName()) {
			if (bLinkAudio)
				path_Audio.setSerializable(false);
			else
				path_Audio.setSerializable(true);
		}

		else if (name == bPlayAudio.getName()) {
			if (!bLinkAudio) {
				if (bPlayAudio) {
		...
		else if (name == bStopAudio.getName()) {
...
public:
	void doOpenDialogPathAudio() {
		ofLogNotice("PlayerAV") << "doOpenDialogPathAudio()";
		...
		if (openFileResult.bSuccess) {
			...
			ofLogNotice("PlayerAV") << "Name: " << name_Audio;
			ofLogNotice("PlayerAV") << "Path: " << path_Audio;
			loadAudio(path_Audio);
		} else {
			ofLogNotice("PlayerAV") << "User hit cancel";
		}
	};

	void doOpenDialogPathVideo() {
		ofLogNotice("PlayerAV") << "doOpenDialogPathVideo()";
		...
		if (openFileResult.bSuccess) {
			...
			ofLogNotice("PlayerAV") << "Name: " << name_Video;
			ofLogNotice("PlayerAV") << "Path: " << path_Video;
			load(path_Video);
		} else {
			ofLogNotice("PlayerAV") << "User hit cancel";
		}
...
public:
	...
	bool load(string path) {
		...
		if (b)
			ofLogNotice("PlayerAV") << "Success: loaded Video File " << path_Video;
		else
			ofLogError("PlayerAV") << "Error: Video File not found" << path_Video;
...
public:
	bool loadAudio(std::string path) {
		...
		if (b)
			ofLogNotice("PlayerAV") << "Success: Loaded Audio file " << path;
		else
			ofLogError("PlayerAV") << "Error: Audio File not found " << path;
